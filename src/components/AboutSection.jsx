...
const RobotIcon = () => <span className="text-4xl md:text-5xl text-orange-500">🤖</span>;
const TrendingUpIcon = () => <span className="text-4xl md:text-5xl text-orange-500">📈</span>;
const UsersIcon = () => <span className="text-4xl md:text-5xl text-orange-500">👥</span>;

// Original icons for "핵심 AI Agent" section
const EmailIcon = () => <span className="text-2xl text-orange-500">📧</span>;
const OnboardingIcon = () => <span className="text-2xl text-orange-500">🚀</span>;
    ...
    >
      <div className="container mx-auto px-6 md:px-12 text-center">
        <div className="mb-16 md:mb-24">
          ...
          <h2 
            className={`text-4xl md:text-6xl lg:text-7xl font-extrabold mb-6 md:mb-8 transition-all duration-700 ease-out delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
          >
            iMate는<br />
          ...
          <p 
            className={`text-lg md:text-xl lg:text-2xl text-slate-600 max-w-3xl mx-auto transition-all duration-700 ease-out delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
          >
            기술과 경험을 바탕으로, 반복적인 비즈니스 업무를 대신하는<br />
        ...
        <div className="mb-16 md:mb-24">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {metricsData.map((metric, index) => (
              <div 
                key={index} 
                className={`p-8 rounded-xl shadow-xl text-left flex flex-col items-start ${metric.bgColor} transform transition-all ease-out duration-500 hover:scale-105 ${metric.delay} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
              >
                <div className={`mb-5 p-3 ${metric.bgColor === 'bg-red-50' ? 'bg-red-100' : metric.bgColor === 'bg-orange-50' ? 'bg-orange-100' : 'bg-amber-100'} rounded-lg`}> {/* Icon background slightly darker shade */}
                  {metric.icon}
                </div>
                <span className={`text-6xl md:text-7xl font-extrabold ${metric.valueColor} mb-3`}>{metric.value}</span>
                <h3 className={`text-xl md:text-2xl font-semibold ${metric.textColor} mb-2`}>{metric.title}</h3>
                <p className="text-sm text-slate-500 leading-relaxed" dangerouslySetInnerHTML={{ __html: metric.description }}></p>
              </div>
            ))}
          </div>
          <p 
            className={`text-sm text-slate-400 mt-10 transition-all duration-700 ease-out delay-[750ms] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
          >
