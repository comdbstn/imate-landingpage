...
const CheckCircleIcon = () => (
  <svg className="w-5 h-5 mr-2.5 text-green-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
  </svg>
);

// Icons for structured content
const ProblemIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
const SolutionIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
const FeatureIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l-3 3m5.657 5.657l-3-3M4.343 15.657l3 3m-5.657-5.657l3 3" /></svg>;
const EffectIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>;
...
const servicesData = [
  {
    ...
    details: {
      ...
      structuredContent: {
        problem: { title: "문제 상황", description: "느리고 반복적인 이메일 응대로 인한 잠재 고객 이탈, 낮은 전환율, 높은 운영 비용, 직원 번아웃" },
        solution: { title: "iMate 솔루션", description: "GPT 기반 AI가 고객 문의 의도를 정확히 파악하여, 24시간 365일 개인화된 답변을 60초 내 자동 전송합니다." },
        features: { title: "주요 기능 및 특징", items: ["지능형 문의 분석 및 분류", "AI 기반 답변 생성 및 추천", "다국어 지원 및 자동 번역", "CRM 및 외부 시스템 연동", "지속적인 학습 및 성능 최적화", "상세 리포팅 및 성과 분석 대시보드"] },
        effects: { title: "도입 효과", items: ["응답 속도 60초 이내로 혁신", "이메일 처리 시간 90% 절감", "고객 만족도 25% 향상", "운영 비용 50% 절감"] }
      }
    }
  },
  {
    ...
    details: {
      ...
      structuredContent: {
        problem: { title: "문제 상황", description: "높은 초기 이탈률, 일관성 없는 온보딩 경험, 반복적인 안내 업무 부담, 낮은 서비스/제품 활성화 비율" },
        solution: { title: "iMate 솔루션", description: "고객 여정 단계별 맞춤형 환영 메시지, 사용 가이드, 튜토리얼, 주요 기능 안내, 성공 팁 등을 자동으로 제공합니다." },
        features: { title: "주요 기능 및 특징", items: ["다채널 온보딩 시나리오 설계", "개인화된 콘텐츠 자동 발송", "고객 행동 기반 조건부 메시징", "핵심 기능 안내 및 사용 유도", "진행 상황 추적 및 리포팅"] },
        effects: { title: "도입 효과", items: ["온보딩 완료율 40% 증가", "초기 이탈률 30% 감소", "지원 문의 60% 감소", "장기 고객 유지율(Retention) 증가"] }
      }
    }
  },
  {
    ...
    details: {
      ...
      structuredContent: {
        problem: { title: "문제 상황", description: "영업 담당자의 바쁜 일정으로 인한 리드 관리 소홀, 잠재 고객과의 관계 단절, 낮은 미팅 전환율, 수동 팔로우업의 비효율" },
        solution: { title: "iMate 솔루션", description: "CRM 데이터와 연동하여 고객의 행동 및 상태 변화에 따라 AI가 개인화된 후속 메시지, 정보, 제안을 적시에 자동 발송합니다." },
        features: { title: "주요 기능 및 특징", items: ["실시간 CRM 데이터 연동 및 동기화", "고객 행동 기반 트리거 설정", "개인화된 메시지 자동 생성 및 발송", "영업 단계별 맞춤 시나리오", "A/B 테스트 및 성과 분석"] },
        effects: { title: "도입 효과", items: ["팔로우업 업무 80% 자동화", "리드 응답률 2배 증가", "미팅 성사율 50% 향상", "영업 주기 평균 15% 단축"] }
      }
    }
  },
  {
    ...
    details: {
      ...
      structuredContent: {
        problem: { title: "문제 상황", description: "낮은 콜드메일 오픈율과 응답률, 스팸으로 간주될 위험, 타겟 고객 분석 및 메시지 작성에 소요되는 많은 시간" },
        solution: { title: "iMate 솔루션", description: "타겟 고객의 최신 정보를 AI가 실시간으로 분석하여, 1:1 맞춤형 콜드메일 초안을 자동으로 생성하고 발송 스케줄링까지 지원합니다." },
        features: { title: "주요 기능 및 특징", items: ["타겟 고객 자동 프로파일링", "AI 기반 초개인화 메시지 생성", "실시간 정보 반영 및 시의성 확보", "발송 스케줄링 및 시퀀스 자동화", "오픈율/응답률 추적 및 성과 분석"] },
        effects: { title: "도입 효과", items: ["콜드메일 작성 시간 90% 절감", "오픈율 평균 40% 이상 달성", "응답률 업계 평균 대비 3~5배 증가", "미팅 전환율 10% 이상 확보"] }
      }
    }
  }
];
          ...
          <div className="space-y-4">
            ...
            {service.details.recommendedIndustries &&
              <div className="mt-6 pt-4 border-t border-slate-200">
                <h4 className="text-xl font-semibold text-slate-800 mb-3">추천 업종</h4>
                <div className="flex flex-wrap gap-2">
                  {service.details.recommendedIndustries.split(',').map(industry => (
                    <span key={industry} className="bg-orange-100 text-orange-700 text-sm font-medium px-3 py-1 rounded-full">{industry.trim()}</span>
                  ))}
                </div>
              </div>
            }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {servicesData.map((service, index) => (
            <div 
              ...
              className={`bg-slate-50 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 flex flex-col justify-between hover:scale-[1.03] overflow-hidden transform cursor-pointer ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="mt-auto">
                  <div
                    className="w-full bg-white hover:bg-orange-500 border-2 border-orange-500 text-orange-500 hover:text-white font-semibold py-2 px-4 rounded-md transition-all duration-200 text-sm text-center"
                  >
                    자세히 보기
                  </div>
                </div>
